 ...
 * @since 1.0.0
 */
public class AProducerTask implements ProducerTask<Long> {
    ...
    public AProducerTask(Long startNumber, Long endNumber, AtomicLong counter, TreeSet<Long> treeSet) {
    ...
    @Override
    public ProducerTask<Long>[] split(int number) throws Exception {
        List<AProducerTask> splitProducers = new ArrayList<>();
        ...
        for (int i = 1; i < splitNumbers.size(); i++) {
            if (i == splitNumbers.size() - 1)
                splitProducers.add(new AProducerTask(splitNumbers.get(i - 1), splitNumbers.get(i), this.counter, this.treeSet));
            else
                splitProducers.add(new AProducerTask(splitNumbers.get(i - 1), splitNumbers.get(i) - 1L, this.counter, this.treeSet));

        }
        if (!CollectionUtils.isEmpty(splitProducers)) {
            return splitProducers.toArray(new AProducerTask[0]);
        }
        return null;
    }
